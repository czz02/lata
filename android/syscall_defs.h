...
#define TARGET_IOC(dir, type, nr, size)                                  \
    (((dir) << TARGET_IOC_DIRSHIFT) | ((type) << TARGET_IOC_TYPESHIFT) | \
     ((nr) << TARGET_IOC_NRSHIFT) | ((size) << TARGET_IOC_SIZESHIFT))

/* used to create numbers */
#define TARGET_IO(type, nr) TARGET_IOC(TARGET_IOC_NONE, (type), (nr), 0)
#define TARGET_IOR(type, nr, size) \
    TARGET_IOC(TARGET_IOC_READ, (type), (nr), sizeof(size))
#define TARGET_IOW(type, nr, size) \
    TARGET_IOC(TARGET_IOC_WRITE, (type), (nr), sizeof(size))
#define TARGET_IOWR(type, nr, size) \
    TARGET_IOC(TARGET_IOC_READ | TARGET_IOC_WRITE, (type), (nr), sizeof(size))

/* the size is automatically computed for these defines */
#define TARGET_IORU(type, nr) \
    TARGET_IOC(TARGET_IOC_READ, (type), (nr), TARGET_IOC_SIZEMASK)
#define TARGET_IOWU(type, nr) \
    TARGET_IOC(TARGET_IOC_WRITE, (type), (nr), TARGET_IOC_SIZEMASK)
#define TARGET_IOWRU(type, nr)                                   \
    TARGET_IOC(TARGET_IOC_READ | TARGET_IOC_WRITE, (type), (nr), \
               TARGET_IOC_SIZEMASK)
...
struct target_sockaddr_in {
    ...
    uint8_t __pad[sizeof(struct target_sockaddr) - sizeof(abi_ushort) -
                  sizeof(abi_short) - sizeof(struct target_in_addr)];
};
...
struct target_msghdr {
    ...
    abi_long
        msg_control; /* Per protocol magic (eg BSD file descriptor passing) */
...
#define TARGET_CMSG_ALIGN(len) \
    (((len) + sizeof(abi_long) - 1) & (size_t)~(sizeof(abi_long) - 1))
#define TARGET_CMSG_SPACE(len) \
    (sizeof(struct target_cmsghdr) + TARGET_CMSG_ALIGN(len))
                     ...
                     struct target_cmsghdr *__cmsg_start)
{
    ...
    __ptr =
        (struct target_cmsghdr *)((unsigned char *)__cmsg +
...
#define TARGET_SI_PAD_SIZE \
    ((TARGET_SI_MAX_SIZE - TARGET_SI_PREAMBLE_SIZE) / sizeof(int))
...
#define TARGET_RTC_PLL_GET TARGET_IOR('p', 0x11, struct target_rtc_pll_info)
#define TARGET_RTC_PLL_SET TARGET_IOW('p', 0x12, struct target_rtc_pll_info)
...
#define TARGET_BLKROSET \
    TARGET_IO(0x12, 93) /* set device read-only (0 = read-write) */
#define TARGET_BLKROGET \
    TARGET_IO(0x12, 94) /* get read-only status (0 = read_write) */
#define TARGET_BLKRRPART TARGET_IO(0x12, 95) /* re-read partition table */
#define TARGET_BLKGETSIZE \
    TARGET_IO(0x12, 96) /* return device size /512 (long *arg) */
#define TARGET_BLKFLSBUF TARGET_IO(0x12, 97) /* flush buffer cache */
#define TARGET_BLKRASET \
    TARGET_IO(0x12, 98) /* Set read ahead for block device */
#define TARGET_BLKRAGET TARGET_IO(0x12, 99) /* get current read ahead setting \
                                             */
#define TARGET_BLKFRASET \
    TARGET_IO(0x12, 100) /* set filesystem (mm/filemap.c) read-ahead */
#define TARGET_BLKFRAGET \
    TARGET_IO(0x12, 101) /* get filesystem (mm/filemap.c) read-ahead */
#define TARGET_BLKSECTSET \
    TARGET_IO(0x12, 102) /* set max sectors per request (ll_rw_blk.c) */
#define TARGET_BLKSECTGET \
    TARGET_IO(0x12, 103) /* get max sectors per request (ll_rw_blk.c) */
#define TARGET_BLKSSZGET TARGET_IO(0x12, 104) /* get block device sector size \
                                               */
#define TARGET_BLKPG \
    TARGET_IO(0x12, 105) /* Partition table and disk geometry handling */
...
#define TARGET_FIGETBSZ \
    TARGET_IO(0x00, 2) /* get the block size used for bmap */
...
#define TARGET_BTRFS_IOC_DEFAULT_SUBVOL \
    TARGET_IOW(BTRFS_IOCTL_MAGIC, 19, abi_ullong)
#define TARGET_BTRFS_IOC_SUBVOL_GETFLAGS \
    TARGET_IOR(BTRFS_IOCTL_MAGIC, 25, abi_ullong)
#define TARGET_BTRFS_IOC_SUBVOL_SETFLAGS \
    TARGET_IOW(BTRFS_IOCTL_MAGIC, 26, abi_ullong)
...
#define TARGET_CDROMPLAYTRKIND       \
    0x5304 /* Play Audio Track/index \
              (struct cdrom_ti) */
#define TARGET_CDROMREADTOCHDR \
    0x5305 /* Read TOC header  \
              (struct cdrom_tochdr) */
#define TARGET_CDROMREADTOCENTRY \
...
#define TARGET_CDROMVOLCTRL         \
    0x530a /* Control output volume \
              (struct cdrom_volctrl) */
#define TARGET_CDROMSUBCHNL        \
    0x530b /* Read subchannel data \
              (struct cdrom_subchnl) */
#define TARGET_CDROMREADMODE2                            \
    0x530c /* Read TARGET_CDROM mode 2 data (2336 Bytes) \
              (struct cdrom_read) */
#define TARGET_CDROMREADMODE1                            \
    0x530d /* Read TARGET_CDROM mode 1 data (2048 Bytes) \
              (struct cdrom_read) */
...
#define TARGET_CDROMMULTISESSION               \
    0x5310 /* Obtain the start-of-last-session \
              address of multi session disks   \
              (struct cdrom_multisession) */
#define TARGET_CDROM_GET_MCN                      \
    0x5311 /* Obtain the "Universal Product Code" \
              if available (struct cdrom_mcn) */
#define TARGET_CDROM_GET_UPC                        \
    TARGET_CDROM_GET_MCN /* This one is deprecated, \
                            but here anyway for compatibility */
#define TARGET_CDROMRESET 0x5312 /* hard-reset the drive */
#define TARGET_CDROMVOLREAD                  \
    0x5313 /* Get the drive's volume setting \
              (struct cdrom_volctrl) */
#define TARGET_CDROMREADRAW                      \
...
#define TARGET_HDIO_GET_MULTCOUNT 0x0304 /* get current IDE blockmode setting \
                                          */
#define TARGET_HDIO_GET_KEEPSETTINGS \
    0x0308 /* get keep-settings-on-reset flag */
...
#if (defined(TARGET_I386) && defined(TARGET_ABI32)) || \
    (defined(TARGET_ARM) && defined(TARGET_ABI32)) || defined(TARGET_CRIS)
...
#define TARGET_SOUND_MIXER_READ_STEREODEVS \
    TARGET_MIXER_READ(SOUND_MIXER_STEREODEVS)
...
#define TARGET_SNDRV_TIMER_IOCTL_GINFO \
    TARGET_IOWR('T', 0x03, struct target_snd_timer_ginfo)
#define TARGET_SNDRV_TIMER_IOCTL_GPARAMS \
    TARGET_IOW('T', 0x04, struct target_snd_timer_gparams)
#define TARGET_SNDRV_TIMER_IOCTL_GSTATUS \
    TARGET_IOWR('T', 0x05, struct target_snd_timer_gstatus)
#define TARGET_SNDRV_TIMER_IOCTL_SELECT \
    TARGET_IOW('T', 0x10, struct target_snd_timer_select)
#define TARGET_SNDRV_TIMER_IOCTL_INFO \
    TARGET_IOR('T', 0x11, struct target_snd_timer_info)
#define TARGET_SNDRV_TIMER_IOCTL_PARAMS \
    TARGET_IOW('T', 0x12, struct snd_timer_params)
#define TARGET_SNDRV_TIMER_IOCTL_STATUS \
    TARGET_IOR('T', 0x14, struct target_snd_timer_status)
...
#else
#define TARGET_SIGEV_PREAMBLE_SIZE \
    (sizeof(abi_int) * 2 + sizeof(target_sigval_t))
#endif

#define TARGET_SIGEV_PAD_SIZE \
    ((TARGET_SIGEV_MAX_SIZE - TARGET_SIGEV_PREAMBLE_SIZE) / sizeof(abi_int))
...
#define TARGET_BINDER_GET_NODE_DEBUG_INFO \
    TARGET_IOWR('b', 11, struct binder_node_debug_info)
#define TARGET_BINDER_GET_NODE_INFO_FOR_REF \
    TARGET_IOWR('b', 12, struct binder_node_info_for_ref)
#define TARGET_BINDER_SET_CONTEXT_MGR_EXT \
    TARGET_IOW('b', 13, struct flat_binder_object)
#define TARGET_BINDER_FREEZE TARGET_IOW('b', 14, struct binder_freeze_info)
#define TARGET_BINDER_GET_FROZEN_INFO \
    TARGET_IOWR('b', 15, struct binder_frozen_status_info)
...
#define TARGET_DMA_HEAP_IOCTL_ALLOC \
    TARGET_IOWR('H', 0, struct dma_heap_allocation_data)
