...
#define SCATTER_OP_PROBE_MEM(TYPE, MMU_IDX, RETADDR)                      \
    do {                                                                  \
        for (int i = 0; i < sizeof(MMVector); i += sizeof(TYPE)) {        \
            if (test_bit(i, env->vtcm_log.mask)) {                        \
                for (int j = 0; j < sizeof(TYPE); j++) {                  \
                    probe_read(env, env->vtcm_log.va[i + j], 1, MMU_IDX,  \
                               RETADDR);                                  \
                    probe_write(env, env->vtcm_log.va[i + j], 1, MMU_IDX, \
                                RETADDR);                                 \
                }                                                         \
            }                                                             \
        }                                                                 \
...
#define fVARRAY_ELEMENT_ACCESS(ARRAY, TYPE, INDEX)            \
    ARRAY.v[(INDEX) / (fVECSIZE() / (sizeof(ARRAY.TYPE[0])))] \
        .TYPE[(INDEX) % (fVECSIZE() / (sizeof(ARRAY.TYPE[0])))]
