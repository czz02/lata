...
#define fGEN_TCG_V6_vhistq(SHORTCODE)                                  \
    do {                                                               \
        if (ctx->pre_commit) {                                         \
            ...
            tcg_gen_gvec_mov(MO_64, dstoff, QvV_off, sizeof(MMVector), \
                             sizeof(MMVector));                        \
        } else {                                                       \
            assert_vhist_tmp(ctx);                                     \
...
#define fGEN_TCG_V6_vassign(SHORTCODE)                          \
    tcg_gen_gvec_mov(MO_64, VdV_off, VuV_off, sizeof(MMVector), \
                     sizeof(MMVector))
...
#define fGEN_TCG_V6_vcombine_tmp(SHORTCODE)                           \
    do {                                                              \
        tcg_gen_gvec_mov(MO_64, VddV_off, VvV_off, sizeof(MMVector),  \
                         sizeof(MMVector));                           \
...
#define fGEN_TCG_V6_vcombine(SHORTCODE)                                   \
    do {                                                                  \
        if (VddV_off != VuV_off) {                                        \
            ...
            tcg_gen_gvec_mov(MO_64, tmpoff, VuV_off, sizeof(MMVector),    \
                             sizeof(MMVector));                           \
            tcg_gen_gvec_mov(MO_64, VddV_off, VvV_off, sizeof(MMVector),  \
                             sizeof(MMVector));                           \
...
#define fGEN_TCG_V6_vcmov(SHORTCODE) fGEN_TCG_VEC_CMOV(1)

/* Vector conditional move (false) */
#define fGEN_TCG_V6_vncmov(SHORTCODE) fGEN_TCG_VEC_CMOV(0)

/* Vector add - various forms */
#define fGEN_TCG_V6_vaddb(SHORTCODE)                                    \
    tcg_gen_gvec_add(MO_8, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vaddh(SHORTCYDE)                                     \
    tcg_gen_gvec_add(MO_16, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vaddw(SHORTCODE)                                     \
    tcg_gen_gvec_add(MO_32, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vaddb_dv(SHORTCODE)                                        \
    tcg_gen_gvec_add(MO_8, VddV_off, VuuV_off, VvvV_off, sizeof(MMVector) * 2, \
                     sizeof(MMVector) * 2)
...
#define fGEN_TCG_V6_vsubb(SHORTCODE)                                    \
    tcg_gen_gvec_sub(MO_8, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vsubh(SHORTCODE)                                     \
    tcg_gen_gvec_sub(MO_16, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vsubw(SHORTCODE)                                     \
    tcg_gen_gvec_sub(MO_32, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vsubb_dv(SHORTCODE)                                        \
    tcg_gen_gvec_sub(MO_8, VddV_off, VuuV_off, VvvV_off, sizeof(MMVector) * 2, \
                     sizeof(MMVector) * 2)
...
#define fGEN_TCG_V6_vasrh(SHORTCODE)                                        \
    do {                                                                    \
        ...
        tcg_gen_gvec_sars(MO_16, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                                \
    } while (0)
...
#define fGEN_TCG_V6_vasrh_acc(SHORTCODE)                                    \
    do {                                                                    \
        ...
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 15);                                    \
        tcg_gen_gvec_sars(MO_16, tmpoff, VuV_off, shift, sizeof(MMVector),  \
                          sizeof(MMVector));                                \
        tcg_gen_gvec_add(MO_16, VxV_off, VxV_off, tmpoff, sizeof(MMVector), \
                         sizeof(MMVector));                                 \
    } while (0)
...
#define fGEN_TCG_V6_vasrw(SHORTCODE)                                        \
    do {                                                                    \
        ...
        tcg_gen_gvec_sars(MO_32, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                                \
    } while (0)
...
#define fGEN_TCG_V6_vasrw_acc(SHORTCODE)                                    \
    do {                                                                    \
        ...
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 31);                                    \
        tcg_gen_gvec_sars(MO_32, tmpoff, VuV_off, shift, sizeof(MMVector),  \
                          sizeof(MMVector));                                \
        tcg_gen_gvec_add(MO_32, VxV_off, VxV_off, tmpoff, sizeof(MMVector), \
                         sizeof(MMVector));                                 \
    } while (0)
...
#define fGEN_TCG_V6_vlsrb(SHORTCODE)                                       \
    do {                                                                   \
        ...
        tcg_gen_gvec_shrs(MO_8, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                               \
    } while (0)
...
#define fGEN_TCG_V6_vlsrh(SHORTCODE)                                        \
    do {                                                                    \
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 15);                                    \
        tcg_gen_gvec_shrs(MO_16, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                                \
    } while (0)
...
#define fGEN_TCG_V6_vlsrw(SHORTCODE)                                        \
    do {                                                                    \
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 31);                                    \
        tcg_gen_gvec_shrs(MO_32, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                                \
    } while (0)
...
#define fGEN_TCG_V6_vaslb(SHORTCODE)                                       \
    do {                                                                   \
        TCGv shift = tcg_temp_new();                                       \
        tcg_gen_andi_tl(shift, RtV, 7);                                    \
        tcg_gen_gvec_shls(MO_8, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                               \
    } while (0)
...
#define fGEN_TCG_V6_vaslh(SHORTCODE)                                        \
    do {                                                                    \
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 15);                                    \
        tcg_gen_gvec_shls(MO_16, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                                \
    } while (0)
...
#define fGEN_TCG_V6_vaslh_acc(SHORTCODE)                                    \
    do {                                                                    \
        ...
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 15);                                    \
        tcg_gen_gvec_shls(MO_16, tmpoff, VuV_off, shift, sizeof(MMVector),  \
                          sizeof(MMVector));                                \
        tcg_gen_gvec_add(MO_16, VxV_off, VxV_off, tmpoff, sizeof(MMVector), \
                         sizeof(MMVector));                                 \
    } while (0)
...
#define fGEN_TCG_V6_vaslw(SHORTCODE)                                        \
    do {                                                                    \
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 31);                                    \
        tcg_gen_gvec_shls(MO_32, VdV_off, VuV_off, shift, sizeof(MMVector), \
                          sizeof(MMVector));                                \
    } while (0)
...
#define fGEN_TCG_V6_vaslw_acc(SHORTCODE)                                    \
    do {                                                                    \
        ...
        TCGv shift = tcg_temp_new();                                        \
        tcg_gen_andi_tl(shift, RtV, 31);                                    \
        tcg_gen_gvec_shls(MO_32, tmpoff, VuV_off, shift, sizeof(MMVector),  \
                          sizeof(MMVector));                                \
        tcg_gen_gvec_add(MO_32, VxV_off, VxV_off, tmpoff, sizeof(MMVector), \
                         sizeof(MMVector));                                 \
    } while (0)
...
#define fGEN_TCG_V6_vmaxw(SHORTCODE)                                      \
    tcg_gen_gvec_smax(MO_32, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vmaxh(SHORTCODE)                                      \
    tcg_gen_gvec_smax(MO_16, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vmaxuh(SHORTCODE)                                     \
    tcg_gen_gvec_umax(MO_16, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vmaxb(SHORTCODE)                                     \
    tcg_gen_gvec_smax(MO_8, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vmaxub(SHORTCODE)                                    \
    tcg_gen_gvec_umax(MO_8, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))

/* Vector min - various forms */
#define fGEN_TCG_V6_vminw(SHORTCODE)                                      \
    tcg_gen_gvec_smin(MO_32, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vminh(SHORTCODE)                                      \
    tcg_gen_gvec_smin(MO_16, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vminuh(SHORTCODE)                                     \
    tcg_gen_gvec_umin(MO_16, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vminb(SHORTCODE)                                     \
    tcg_gen_gvec_smin(MO_8, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))
#define fGEN_TCG_V6_vminub(SHORTCODE)                                    \
    tcg_gen_gvec_umin(MO_8, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                      sizeof(MMVector))

/* Vector logical ops */
#define fGEN_TCG_V6_vxor(SHORTCODE)                                      \
    tcg_gen_gvec_xor(MO_64, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vand(SHORTCODE)                                      \
    tcg_gen_gvec_and(MO_64, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vor(SHORTCODE)                                      \
    tcg_gen_gvec_or(MO_64, VdV_off, VuV_off, VvV_off, sizeof(MMVector), \
                    sizeof(MMVector))

#define fGEN_TCG_V6_vnot(SHORTCODE)                             \
    tcg_gen_gvec_not(MO_64, VdV_off, VuV_off, sizeof(MMVector), \
                     sizeof(MMVector))

/* Q register logical ops */
#define fGEN_TCG_V6_pred_or(SHORTCODE)                                \
    tcg_gen_gvec_or(MO_64, QdV_off, QsV_off, QtV_off, sizeof(MMQReg), \
                    sizeof(MMQReg))

#define fGEN_TCG_V6_pred_and(SHORTCODE)                                \
    tcg_gen_gvec_and(MO_64, QdV_off, QsV_off, QtV_off, sizeof(MMQReg), \
                     sizeof(MMQReg))

#define fGEN_TCG_V6_pred_xor(SHORTCODE)                                \
    tcg_gen_gvec_xor(MO_64, QdV_off, QsV_off, QtV_off, sizeof(MMQReg), \
                     sizeof(MMQReg))

#define fGEN_TCG_V6_pred_or_n(SHORTCODE)                               \
    tcg_gen_gvec_orc(MO_64, QdV_off, QsV_off, QtV_off, sizeof(MMQReg), \
                     sizeof(MMQReg))

#define fGEN_TCG_V6_pred_and_n(SHORTCODE)                               \
    tcg_gen_gvec_andc(MO_64, QdV_off, QsV_off, QtV_off, sizeof(MMQReg), \
                      sizeof(MMQReg))

#define fGEN_TCG_V6_pred_not(SHORTCODE) \
    tcg_gen_gvec_not(MO_64, QdV_off, QsV_off, sizeof(MMQReg), sizeof(MMQReg))
...
#define fGEN_TCG_V6_vgtw(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_GT, MO_32, 4)
#define fGEN_TCG_V6_vgth(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_GT, MO_16, 2)
#define fGEN_TCG_V6_vgtb(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_GT, MO_8, 1)

#define fGEN_TCG_V6_vgtuw(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_GTU, MO_32, 4)
#define fGEN_TCG_V6_vgtuh(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_GTU, MO_16, 2)
#define fGEN_TCG_V6_vgtub(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_GTU, MO_8, 1)

#define fGEN_TCG_V6_veqw(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_EQ, MO_32, 4)
#define fGEN_TCG_V6_veqh(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_EQ, MO_16, 2)
#define fGEN_TCG_V6_veqb(SHORTCODE) fGEN_TCG_VEC_CMP(TCG_COND_EQ, MO_8, 1)
...
#define fGEN_TCG_V6_lvsplatw(SHORTCODE)                                      \
    tcg_gen_gvec_dup_i32(MO_32, VdV_off, sizeof(MMVector), sizeof(MMVector), \
                         RtV)

#define fGEN_TCG_V6_lvsplath(SHORTCODE)                                      \
    tcg_gen_gvec_dup_i32(MO_16, VdV_off, sizeof(MMVector), sizeof(MMVector), \
                         RtV)

#define fGEN_TCG_V6_lvsplatb(SHORTCODE) \
    tcg_gen_gvec_dup_i32(MO_8, VdV_off, sizeof(MMVector), sizeof(MMVector), RtV)

/* Vector absolute value - various forms */
#define fGEN_TCG_V6_vabsb(SHORTCODE) \
    tcg_gen_gvec_abs(MO_8, VdV_off, VuV_off, sizeof(MMVector), sizeof(MMVector))

#define fGEN_TCG_V6_vabsh(SHORTCODE)                            \
    tcg_gen_gvec_abs(MO_16, VdV_off, VuV_off, sizeof(MMVector), \
                     sizeof(MMVector))

#define fGEN_TCG_V6_vabsw(SHORTCODE)                            \
    tcg_gen_gvec_abs(MO_32, VdV_off, VuV_off, sizeof(MMVector), \
                     sizeof(MMVector))
...
#define fGEN_TCG_PRED_VEC_LOAD_pred_pi                          \
    fGEN_TCG_PRED_VEC_LOAD(fLSBOLD(PvV), fEA_REG(RxV), VdV_off, \
                           fPM_I(RxV, siV * sizeof(MMVector)))
#define fGEN_TCG_PRED_VEC_LOAD_npred_pi                            \
    fGEN_TCG_PRED_VEC_LOAD(fLSBOLDNOT(PvV), fEA_REG(RxV), VdV_off, \
                           fPM_I(RxV, siV * sizeof(MMVector)))

#define fGEN_TCG_V6_vL32b_pred_pi(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_pi
#define fGEN_TCG_V6_vL32b_npred_pi(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_npred_pi
#define fGEN_TCG_V6_vL32b_cur_pred_pi(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_pi
#define fGEN_TCG_V6_vL32b_cur_npred_pi(SHORTCODE) \
    fGEN_TCG_PRED_VEC_LOAD_npred_pi
#define fGEN_TCG_V6_vL32b_tmp_pred_pi(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_pi
#define fGEN_TCG_V6_vL32b_tmp_npred_pi(SHORTCODE) \
    fGEN_TCG_PRED_VEC_LOAD_npred_pi
#define fGEN_TCG_V6_vL32b_nt_pred_pi(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_pi
#define fGEN_TCG_V6_vL32b_nt_npred_pi(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_npred_pi
...
#define fGEN_TCG_PRED_VEC_LOAD_pred_ai                              \
    fGEN_TCG_PRED_VEC_LOAD(                                         \
        fLSBOLD(PvV), fEA_RI(RtV, siV * sizeof(MMVector)), VdV_off, \
        do {} while (0))
#define fGEN_TCG_PRED_VEC_LOAD_npred_ai                                \
    fGEN_TCG_PRED_VEC_LOAD(                                            \
        fLSBOLDNOT(PvV), fEA_RI(RtV, siV * sizeof(MMVector)), VdV_off, \
        do {} while (0))

#define fGEN_TCG_V6_vL32b_pred_ai(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_ai
#define fGEN_TCG_V6_vL32b_npred_ai(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_npred_ai
#define fGEN_TCG_V6_vL32b_cur_pred_ai(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_ai
#define fGEN_TCG_V6_vL32b_cur_npred_ai(SHORTCODE) \
    fGEN_TCG_PRED_VEC_LOAD_npred_ai
#define fGEN_TCG_V6_vL32b_tmp_pred_ai(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_ai
#define fGEN_TCG_V6_vL32b_tmp_npred_ai(SHORTCODE) \
    fGEN_TCG_PRED_VEC_LOAD_npred_ai
#define fGEN_TCG_V6_vL32b_nt_pred_ai(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_ai
#define fGEN_TCG_V6_vL32b_nt_npred_ai(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_npred_ai
...
#define fGEN_TCG_PRED_VEC_LOAD_pred_ppu \
    fGEN_TCG_PRED_VEC_LOAD(fLSBOLD(PvV), fEA_REG(RxV), VdV_off, fPM_M(RxV, MuV))
#define fGEN_TCG_PRED_VEC_LOAD_npred_ppu                           \
    fGEN_TCG_PRED_VEC_LOAD(fLSBOLDNOT(PvV), fEA_REG(RxV), VdV_off, \
                           fPM_M(RxV, MuV))

#define fGEN_TCG_V6_vL32b_pred_ppu(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_ppu
#define fGEN_TCG_V6_vL32b_npred_ppu(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_npred_ppu
...
#define fGEN_TCG_V6_vL32b_tmp_npred_ppu(SHORTCODE) \
    fGEN_TCG_PRED_VEC_LOAD_npred_ppu
#define fGEN_TCG_V6_vL32b_nt_pred_ppu(SHORTCODE) fGEN_TCG_PRED_VEC_LOAD_pred_ppu
...
#define fGEN_TCG_V6_vS32b_new_pi(SHORTCODE) fGEN_TCG_NEWVAL_VEC_STORE_pi
#define fGEN_TCG_V6_vS32b_nt_new_pi(SHORTCODE) fGEN_TCG_NEWVAL_VEC_STORE_pi

#define fGEN_TCG_NEWVAL_VEC_STORE_ai \
    fGEN_TCG_NEWVAL_VEC_STORE(       \
        fEA_RI(RtV, siV * sizeof(MMVector)), do {} while (0))

#define fGEN_TCG_V6_vS32b_new_ai(SHORTCODE) fGEN_TCG_NEWVAL_VEC_STORE_ai
#define fGEN_TCG_V6_vS32b_nt_new_ai(SHORTCODE) fGEN_TCG_NEWVAL_VEC_STORE_ai

#define fGEN_TCG_NEWVAL_VEC_STORE_ppu \
    fGEN_TCG_NEWVAL_VEC_STORE(fEA_REG(RxV), fPM_M(RxV, MuV))

#define fGEN_TCG_V6_vS32b_new_ppu(SHORTCODE) fGEN_TCG_NEWVAL_VEC_STORE_ppu
#define fGEN_TCG_V6_vS32b_nt_new_ppu(SHORTCODE) fGEN_TCG_NEWVAL_VEC_STORE_ppu
...
#define fGEN_TCG_PRED_VEC_STORE_pred_pi(ALIGN)                          \
    fGEN_TCG_PRED_VEC_STORE(fLSBOLD(PvV), fEA_REG(RxV), VsV_off, ALIGN, \
                            fPM_I(RxV, siV * sizeof(MMVector)))
#define fGEN_TCG_PRED_VEC_STORE_npred_pi(ALIGN)                            \
    fGEN_TCG_PRED_VEC_STORE(fLSBOLDNOT(PvV), fEA_REG(RxV), VsV_off, ALIGN, \
                            fPM_I(RxV, siV * sizeof(MMVector)))
#define fGEN_TCG_PRED_VEC_STORE_new_pred_pi                            \
    fGEN_TCG_PRED_VEC_STORE(fLSBOLD(PvV), fEA_REG(RxV), OsN_off, true, \
                            fPM_I(RxV, siV * sizeof(MMVector)))
#define fGEN_TCG_PRED_VEC_STORE_new_npred_pi                              \
    fGEN_TCG_PRED_VEC_STORE(fLSBOLDNOT(PvV), fEA_REG(RxV), OsN_off, true, \
                            fPM_I(RxV, siV * sizeof(MMVector)))
...
#define fGEN_TCG_PRED_VEC_STORE_pred_ai(ALIGN)                             \
    fGEN_TCG_PRED_VEC_STORE(                                               \
        fLSBOLD(PvV), fEA_RI(RtV, siV * sizeof(MMVector)), VsV_off, ALIGN, \
        do {} while (0))
#define fGEN_TCG_PRED_VEC_STORE_npred_ai(ALIGN)                               \
    fGEN_TCG_PRED_VEC_STORE(                                                  \
        fLSBOLDNOT(PvV), fEA_RI(RtV, siV * sizeof(MMVector)), VsV_off, ALIGN, \
        do {} while (0))
#define fGEN_TCG_PRED_VEC_STORE_new_pred_ai                               \
    fGEN_TCG_PRED_VEC_STORE(                                              \
        fLSBOLD(PvV), fEA_RI(RtV, siV * sizeof(MMVector)), OsN_off, true, \
        do {} while (0))
#define fGEN_TCG_PRED_VEC_STORE_new_npred_ai                                 \
    fGEN_TCG_PRED_VEC_STORE(                                                 \
        fLSBOLDNOT(PvV), fEA_RI(RtV, siV * sizeof(MMVector)), OsN_off, true, \
        do {} while (0))
