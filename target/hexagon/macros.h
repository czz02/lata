...
#define SET_USR_FIELD(FIELD, VAL)                                            \
    do {                                                                     \
        if (pkt_need_commit) {                                               \
            fINSERT_BITS(env->new_value_usr, reg_field_info[FIELD].width,    \
...
#define MEM_STORE1_FUNC(X)        \
    __builtin_choose_expr(        \
        TYPE_INT(X), gen_store1i, \
        __builtin_choose_expr(TYPE_TCGV(X), gen_store1, (void)0))
#define MEM_STORE1(VA, DATA, SLOT) \
    MEM_STORE1_FUNC(DATA)(cpu_env, VA, DATA, SLOT)

#define MEM_STORE2_FUNC(X)        \
    __builtin_choose_expr(        \
        TYPE_INT(X), gen_store2i, \
        __builtin_choose_expr(TYPE_TCGV(X), gen_store2, (void)0))
#define MEM_STORE2(VA, DATA, SLOT) \
    MEM_STORE2_FUNC(DATA)(cpu_env, VA, DATA, SLOT)

#define MEM_STORE4_FUNC(X)        \
    __builtin_choose_expr(        \
        TYPE_INT(X), gen_store4i, \
        __builtin_choose_expr(TYPE_TCGV(X), gen_store4, (void)0))
#define MEM_STORE4(VA, DATA, SLOT) \
    MEM_STORE4_FUNC(DATA)(cpu_env, VA, DATA, SLOT)

#define MEM_STORE8_FUNC(X)        \
    __builtin_choose_expr(        \
        TYPE_INT(X), gen_store8i, \
        __builtin_choose_expr(TYPE_TCGV_I64(X), gen_store8, (void)0))
...
#define LOAD_CANCEL(EA) \
    do {                \
        CANCEL;         \
    } while (0)

#define STORE_CANCEL(EA)                    \
    {                                       \
...
#define fINSERT_RANGE(INREG, HIBIT, LOWBIT, INVAL)                           \
    do {                                                                     \
        ...
        INREG = (width >= 0 ? deposit64((INREG), (LOWBIT), width, (INVAL)) : \
                              INREG);                                        \
    } while (0)
...
#define fBIDIR_ASHIFTR_SAT(SRC, SHAMT, REGSTYPE)                           \
    (((SHAMT) < 0) ?                                                       \
         fSAT_ORIG_SHL((fCAST##REGSTYPE##s(SRC) << ((-(SHAMT)) - 1)) << 1, \
                       (SRC)) :                                            \
...
#define fFRAMECHECK(ADDR, EA) \
    do {                      \
    } while (0) /* Not modelled in linux-user */
#else
...
#define GETBYTE_FUNC(X)             \
    __builtin_choose_expr(          \
        TYPE_TCGV(X), gen_get_byte, \
        __builtin_choose_expr(TYPE_TCGV_I64(X), gen_get_byte_i64, (void)0))
...
#define fREAD_REG_FIELD(REG, FIELD)                                      \
    fEXTRACTU_BITS(env->gpr[HEX_REG_##REG], reg_field_info[FIELD].width, \
                   reg_field_info[FIELD].offset)
