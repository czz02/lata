...
#define spr_register_kvm_hv(env, num, name, uea_read, uea_write, oea_read,    \
                            oea_write, hea_read, hea_write, one_reg_id,       \
                            initial_value)                                    \
    _spr_register(env, num, name,                                             \
                  USR_ARG(uea_read) USR_ARG(uea_write) SYS_ARG(oea_read)      \
                      SYS_ARG(oea_write) SYS_ARG(hea_read) SYS_ARG(hea_write) \
                          KVM_ARG(one_reg_id) initial_value)

/* spr_register_kvm duplicates the oea callbacks to the hea callbacks. */
#define spr_register_kvm(env, num, name, uea_read, uea_write, oea_read, \
                         oea_write, one_reg_id, ival)                   \
...
#define spr_register_hv(env, num, name, uea_read, uea_write, oea_read, \
                        oea_write, hea_read, hea_write, ival)          \
    spr_register_kvm_hv(env, num, name, uea_read, uea_write, oea_read, \
                        oea_write, hea_read, hea_write, 0, ival)

#define spr_register(env, num, name, uea_read, uea_write, oea_read, oea_write, \
                     ival)                                                     \
    spr_register_kvm(env, num, name, uea_read, uea_write, oea_read, oea_write, \
                     0, ival)
