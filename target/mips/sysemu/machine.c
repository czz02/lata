...
static VMStateField vmstate_fpu_fields[] = {
    ...
    VMSTATE_UINT32(fcr31, CPUMIPSFPUContext), VMSTATE_END_OF_LIST()
};
...
const VMStateDescription vmstate_mvp = {
    ...
    .minimum_version_id = 1,
    .fields =
        (VMStateField[]){ VMSTATE_INT32(CP0_MVPControl, CPUMIPSMVPContext),
                   ...
                   const VMStateField *field, JSONWriter *vmdesc)
{
    ...
    uint16_t flags =
        ((v->EHINV << 15) | (v->RI1 << 14) | (v->RI0 << 13) | (v->XI1 << 12) |
         (v->XI0 << 11) | (v->G << 10) | (v->C0 << 7) | (v->C1 << 4) |
         (v->V0 << 3) | (v->V1 << 2) | (v->D0 << 1) | (v->D1 << 0));
