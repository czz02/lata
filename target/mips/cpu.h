...
#define SET_FP_COND(num, env)                                       \
    do {                                                            \
        ((env).fcr31) |= ((num) ? (1 << ((num) + 24)) : (1 << 23)); \
    } while (0)
#define CLEAR_FP_COND(num, env)                                      \
    do {                                                             \
        ((env).fcr31) &= ~((num) ? (1 << ((num) + 24)) : (1 << 23)); \
    } while (0)
#define GET_FP_COND(env) \
    ((((env).fcr31 >> 24) & 0xfe) | (((env).fcr31 >> 23) & 0x1))
...
#define SET_FP_ENABLE(reg, v)                               \
    do {                                                    \
        (reg) = ((reg) & ~(0x1f << 7)) | ((v & 0x1f) << 7); \
    } while (0)
#define SET_FP_FLAGS(reg, v)                                \
    do {                                                    \
        (reg) = ((reg) & ~(0x1f << 2)) | ((v & 0x1f) << 2); \
    } while (0)
...
#define MSACSR_MASK \
    (MSACSR_RM_MASK | MSACSR_CEF_MASK | MSACSR_NX_MASK | MSACSR_FS_MASK)
...
#define CP0SC_MASK \
    (CP0SC_C_MASK | CP0SC_EU_MASK | CP0SC_AM_MASK | CP0SC_PA_MASK)
#define CP0SC_1GMASK \
    (CP0SC_C_MASK | CP0SC_EU_MASK | CP0SC_AM_MASK | CP0SC_PA_1GMASK)
