...
void HELPER(vhaddw_q_d)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                        uint32_t vk)
{
    VReg *Vd = &(env->fpr[vd].vreg);
    VReg *Vj = &(env->fpr[vj].vreg);
...
void HELPER(vhsubw_q_d)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                        uint32_t vk)
{
    VReg *Vd = &(env->fpr[vd].vreg);
    VReg *Vj = &(env->fpr[vj].vreg);
...
void HELPER(vhaddw_qu_du)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                          uint32_t vk)
{
    VReg *Vd = &(env->fpr[vd].vreg);
    VReg *Vj = &(env->fpr[vj].vreg);
...
void HELPER(vhsubw_qu_du)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                          uint32_t vk)
{
    VReg *Vd = &(env->fpr[vd].vreg);
    VReg *Vj = &(env->fpr[vj].vreg);
...
#define DO_ODD_U_S(NAME, BIT, ES1, EU1, ES2, EU2, DO_OP)                 \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->ES1(i) =                                                 \
                DO_OP((TDU)Vj->EU2(2 * i + 1), (TDS)Vk->ES2(2 * i + 1)); \
        }                                                                \
    }
...
void HELPER(vaddwev_q_du_d)(void *vd, void *vj, void *vk, uint32_t v)
{
    ...
    Vd->Q(0) =
        int128_add(int128_make64((uint64_t)Vj->D(0)), int128_makes64(Vk->D(0)));
}
...
void HELPER(vaddwod_q_du_d)(void *vd, void *vj, void *vk, uint32_t v)
{
    ...
    Vd->Q(0) =
        int128_add(int128_make64((uint64_t)Vj->D(1)), int128_makes64(Vk->D(1)));
}
...
#define VMADDWEV_U_S(NAME, BIT, ES1, EU1, ES2, EU2, DO_OP)                 \
    ...
    {                                                                      \
        ...
        typedef __typeof(Vd->EU1(0)) TU1;                                  \
                                                                           \
        for (i = 0; i < LSX_LEN / BIT; i++) {                              \
            Vd->ES1(i) += DO_OP((TU1)Vj->EU2(2 * i), (TS1)Vk->ES2(2 * i)); \
        }                                                                  \
    }
...
#define VMADDWOD_U_S(NAME, BIT, ES1, EU1, ES2, EU2, DO_OP)               \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->ES1(i) +=                                                \
                DO_OP((TU1)Vj->EU2(2 * i + 1), (TS1)Vk->ES2(2 * i + 1)); \
        }                                                                \
    }
...
#define DO_DIV(N, M)                                       \
    (unlikely(M == 0)                                ? 0 : \
     unlikely((N == -N) && (M == (__typeof(N))(-1))) ? N : \
                                                       N / M)
#define DO_REM(N, M)                                       \
    (unlikely(M == 0)                                ? 0 : \
     unlikely((N == -N) && (M == (__typeof(N))(-1))) ? 0 : \
                                                       N % M)
...
do_vsrlr(B, uint8_t) do_vsrlr(H, uint16_t) do_vsrlr(W,
                                                    uint32_t) do_vsrlr(D,
                                                                       uint64_t)
...
#define VSRLR(NAME, BIT, T, E)                                           \
    ...
    VSRLR(vsrlr_b, 8, uint8_t, B) VSRLR(vsrlr_h, 16, uint16_t, H) VSRLR(
        vsrlr_w, 32, uint32_t, W) VSRLR(vsrlr_d, 64, uint64_t, D)
...
#define VSRLRI(NAME, BIT, E)                                             \
    ...
    {                                                                    \
        ...
        VSRLRI(vsrlri_b, 8, B) VSRLRI(vsrlri_h, 16,
                                      H) VSRLRI(vsrlri_w, 32,
                                                W) VSRLRI(vsrlri_d, 64, D)
...
#define do_vsrar(E, T)                                    \
    ...
    {                                                     \
        if (sh == 0) {                                    \
            ...
            do_vsrar(B, int8_t) do_vsrar(H, int16_t) do_vsrar(W, int32_t)
                do_vsrar(D, int64_t)
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->E(i) = do_vsrar_##E(Vj->E(i), ((T)Vk->E(i)) % BIT);      \
                    ...
                    VSRAR(vsrar_b, 8, uint8_t,
                          B) VSRAR(vsrar_h, 16, uint16_t,
                                   H) VSRAR(vsrar_w, 32, uint32_t,
                                            W) VSRAR(vsrar_d, 64, uint64_t, D)
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->E(i) = do_vsrar_##E(Vj->E(i), imm);                      \
                        ...
                        VSRARI(vsrari_b, 8, B) VSRARI(vsrari_h, 16, H)
                            VSRARI(vsrari_w, 32, W) VSRARI(vsrari_d, 64, D)
...
#define VSRLN(NAME, BIT, T, E1, E2)                                      \
    ...
    {                                                                    \
        ...
        Vd->D(1) = 0;                                                    \
                                ...
                                VSRLN(vsrln_b_h, 16, uint16_t, B,
                                      H) VSRLN(vsrln_h_w, 32, uint32_t, H, W)
    ...
    {                                                                    \
        ...
        Vd->D(1) = 0;                                                    \
                                        ...
                                        VSRAN(vsran_b_h, 16, uint16_t, B, H)
                                            VSRAN(vsran_h_w, 32, uint32_t, H,
                                                  W) VSRAN(vsran_w_d, 64,
                                                           uint64_t, W, D)
...
void HELPER(vsrani_d_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                        uint32_t imm)
{
    VReg temp;
    VReg *Vd = &(env->fpr[vd].vreg);
...
void HELPER(vsrlrni_d_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                         uint32_t imm)
{
    VReg temp;
    VReg *Vd = &(env->fpr[vd].vreg);
...
void HELPER(vsrarni_d_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                         uint32_t imm)
{
    VReg temp;
    VReg *Vd = &(env->fpr[vd].vreg);
...
#define VSSRLN(NAME, BIT, T, E1, E2)                                        \
    ...
    {                                                                       \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                               \
            Vd->E1(i) =                                                     \
                do_ssrlns_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2 - 1); \
        }                                                                   \
        Vd->D(1) = 0;                                                       \
    }
...
#define VSSRAN(NAME, BIT, T, E1, E2)                                        \
    ...
    {                                                                       \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                               \
            Vd->E1(i) =                                                     \
                do_ssrans_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2 - 1); \
        }                                                                   \
        Vd->D(1) = 0;                                                       \
    }
...
#define VSSRLNU(NAME, BIT, T, E1, E2)                                    \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->E1(i) =                                                  \
                do_ssrlnu_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2);  \
        }                                                                \
        Vd->D(1) = 0;                                                    \
    }
...
#define VSSRANU(NAME, BIT, T, E1, E2)                                    \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->E1(i) =                                                  \
                do_ssranu_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2);  \
        }                                                                \
        Vd->D(1) = 0;                                                    \
    }
...
#define VSSRLNI(NAME, BIT, E1, E2)                                       \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrlns_##E1(Vd->E2(i), imm, BIT / 2 - 1);             \
        }                                                                \
        *Vd = temp;                                                      \
    }

void HELPER(vssrlni_d_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                         uint32_t imm)
{
...
#define VSSRANI(NAME, BIT, E1, E2)                                       \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrans_##E1(Vd->E2(i), imm, BIT / 2 - 1);             \
        }                                                                \
        *Vd = temp;                                                      \
    }

void HELPER(vssrani_d_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                         uint32_t imm)
{
...
#define VSSRLNUI(NAME, BIT, E1, E2)                                      \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrlnu_##E1(Vd->E2(i), imm, BIT / 2);                 \
        }                                                                \
        *Vd = temp;                                                      \
    }

void HELPER(vssrlni_du_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                          uint32_t imm)
{
    Int128 shft_res1, shft_res2, mask;
    VReg *Vd = &(env->fpr[vd].vreg);
...
#define VSSRANUI(NAME, BIT, E1, E2)                                      \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssranu_##E1(Vd->E2(i), imm, BIT / 2);                 \
        }                                                                \
        *Vd = temp;                                                      \
    }

void HELPER(vssrani_du_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                          uint32_t imm)
{
    Int128 shft_res1, shft_res2, mask;
    VReg *Vd = &(env->fpr[vd].vreg);
...
#define VSSRLRN(NAME, BIT, T, E1, E2)                                        \
    ...
    {                                                                        \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                                \
            Vd->E1(i) =                                                      \
                do_ssrlrns_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2 - 1); \
        }                                                                    \
        Vd->D(1) = 0;                                                        \
    }
...
#define VSSRARN(NAME, BIT, T, E1, E2)                                        \
    ...
    {                                                                        \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                                \
            Vd->E1(i) =                                                      \
                do_ssrarns_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2 - 1); \
        }                                                                    \
        Vd->D(1) = 0;                                                        \
    }
...
#define VSSRLRNU(NAME, BIT, T, E1, E2)                                   \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->E1(i) =                                                  \
                do_ssrlrnu_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2); \
        }                                                                \
        Vd->D(1) = 0;                                                    \
    }
...
#define VSSRARNU(NAME, BIT, T, E1, E2)                                   \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            Vd->E1(i) =                                                  \
                do_ssrarnu_##E1(Vj->E2(i), (T)Vk->E2(i) % BIT, BIT / 2); \
        }                                                                \
        Vd->D(1) = 0;                                                    \
    }
...
#define VSSRLRNI(NAME, BIT, E1, E2)                                      \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrlrns_##E1(Vd->E2(i), imm, BIT / 2 - 1);            \
        }                                                                \
        *Vd = temp;                                                      \
    }
...
#define VSSRLRNI_Q(NAME, sh)                                                   \
    ...
    {                                                                          \
        ...
        if (imm == 0) {                                                        \
            ...
            r1 =                                                               \
                int128_and(int128_urshift(Vj->Q(0), (imm - 1)), int128_one()); \
            r2 =                                                               \
                int128_and(int128_urshift(Vd->Q(0), (imm - 1)), int128_one()); \
                                                                               \
...
#define VSSRARNI(NAME, BIT, E1, E2)                                      \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrarns_##E1(Vd->E2(i), imm, BIT / 2 - 1);            \
        }                                                                \
        *Vd = temp;                                                      \
    }

void HELPER(vssrarni_d_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                          uint32_t imm)
{
...
#define VSSRLRNUI(NAME, BIT, E1, E2)                                     \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrlrnu_##E1(Vd->E2(i), imm, BIT / 2);                \
        }                                                                \
        *Vd = temp;                                                      \
    }
...
#define VSSRARNUI(NAME, BIT, E1, E2)                                     \
    ...
    {                                                                    \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                            \
            ...
            temp.E1(i + LSX_LEN / BIT) =                                 \
                do_ssrarnu_##E1(Vd->E2(i), imm, BIT / 2);                \
        }                                                                \
        *Vd = temp;                                                      \
    }

void HELPER(vssrarni_du_q)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                           uint32_t imm)
{
    Int128 shft_res1, shft_res2, mask1, mask2, r1, r2;
    VReg *Vd = &(env->fpr[vd].vreg);
...
static void vec_update_fcsr0_mask(CPULoongArchState *env, uintptr_t pc,
                                  int mask)
{
...
#define DO_4OP_F(NAME, BIT, E, FN, flags)                                 \
    ...
    {                                                                     \
        ...
        for (i = 0; i < LSX_LEN / BIT; i++) {                             \
            Vd->E(i) =                                                    \
                FN(Vj->E(i), Vk->E(i), Va->E(i), flags, &env->fp_status); \
            vec_update_fcsr0(env, GETPC());                               \
        }                                                                 \
    }
...
void HELPER(vfcvt_h_s)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                       uint32_t vk)
{
    int i;
    VReg temp;
...
void HELPER(vfcvt_s_d)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                       uint32_t vk)
{
    int i;
    VReg temp;
...
void HELPER(vffint_s_l)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                        uint32_t vk)
{
    int i;
    VReg temp;
...
static uint64_t vfcmp_common(CPULoongArchState *env, FloatRelation cmp,
                             uint32_t flags)
{
    uint64_t ret = 0;
...
void HELPER(vshuf_b)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                     uint32_t vk, uint32_t va)
{
    int i, m;
    VReg temp;
...
void HELPER(vshuf4i_d)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                       uint32_t imm)
{
    VReg *Vd = &(env->fpr[vd].vreg);
    VReg *Vj = &(env->fpr[vj].vreg);
...
void HELPER(vpermi_w)(CPULoongArchState *env, uint32_t vd, uint32_t vj,
                      uint32_t imm)
{
    VReg temp;
    VReg *Vd = &(env->fpr[vd].vreg);
