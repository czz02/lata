...
#define CSR_NAME(REG) [LOONGARCH_CSR_##REG] = (#REG)

static const char *const csr_names[] = {
    CSR_NAME(CRMD),      CSR_NAME(PRMD),      CSR_NAME(EUEN),
    CSR_NAME(MISC),      CSR_NAME(ECFG),      CSR_NAME(ESTAT),
    CSR_NAME(ERA),       CSR_NAME(BADV),      CSR_NAME(BADI),
    CSR_NAME(EENTRY),    CSR_NAME(TLBIDX),    CSR_NAME(TLBEHI),
    CSR_NAME(TLBELO0),   CSR_NAME(TLBELO1),   CSR_NAME(ASID),
    CSR_NAME(PGDL),      CSR_NAME(PGDH),      CSR_NAME(PGD),
    CSR_NAME(PWCL),      CSR_NAME(PWCH),      CSR_NAME(STLBPS),
    CSR_NAME(RVACFG),    CSR_NAME(CPUID),     CSR_NAME(PRCFG1),
    CSR_NAME(PRCFG2),    CSR_NAME(PRCFG3),    CSR_NAME(SAVE(0)),
    CSR_NAME(SAVE(1)),   CSR_NAME(SAVE(2)),   CSR_NAME(SAVE(3)),
    CSR_NAME(SAVE(4)),   CSR_NAME(SAVE(5)),   CSR_NAME(SAVE(6)),
    CSR_NAME(SAVE(7)),   CSR_NAME(SAVE(8)),   CSR_NAME(SAVE(9)),
    CSR_NAME(SAVE(10)),  CSR_NAME(SAVE(11)),  CSR_NAME(SAVE(12)),
    CSR_NAME(SAVE(13)),  CSR_NAME(SAVE(14)),  CSR_NAME(SAVE(15)),
    CSR_NAME(TID),       CSR_NAME(TCFG),      CSR_NAME(TVAL),
    CSR_NAME(CNTC),      CSR_NAME(TICLR),     CSR_NAME(LLBCTL),
    CSR_NAME(IMPCTL1),   CSR_NAME(IMPCTL2),   CSR_NAME(TLBRENTRY),
    CSR_NAME(TLBRBADV),  CSR_NAME(TLBRERA),   CSR_NAME(TLBRSAVE),
    CSR_NAME(TLBRELO0),  CSR_NAME(TLBRELO1),  CSR_NAME(TLBREHI),
    CSR_NAME(TLBRPRMD),  CSR_NAME(MERRCTL),   CSR_NAME(MERRINFO1),
    CSR_NAME(MERRINFO2), CSR_NAME(MERRENTRY), CSR_NAME(MERRERA),
    CSR_NAME(MERRSAVE),  CSR_NAME(CTAG),      CSR_NAME(DMW(0)),
    CSR_NAME(DMW(1)),    CSR_NAME(DMW(2)),    CSR_NAME(DMW(3)),
    CSR_NAME(DBG),       CSR_NAME(DERA),      CSR_NAME(DSAVE),
};
...
int print_insn_loongarch(bfd_vma memaddr, struct disassemble_info *info)
{
    ...
    DisasContext ctx = { .info = info, .pc = memaddr, .insn = insn };
...
static void output_rr_jk(DisasContext *ctx, arg_rr_jk *a, const char *mnemonic)
{
...
static void output_offs(DisasContext *ctx, arg_offs *a, const char *mnemonic)
{
...
static void output_rr_offs(DisasContext *ctx, arg_rr_offs *a,
                           const char *mnemonic)
{
    output(ctx, mnemonic, "r%d, r%d, %d # 0x%" PRIx64, a->rj, a->rd, a->offs,
           ctx->pc + a->offs);
}

static void output_r_csr(DisasContext *ctx, arg_r_csr *a, const char *mnemonic)
{
...
static void output_rr_csr(DisasContext *ctx, arg_rr_csr *a,
                          const char *mnemonic)
{
    output(ctx, mnemonic, "r%d, r%d, %d # %s", a->rd, a->rj, a->csr,
           get_csr_name(a->csr));
}

static void output_empty(DisasContext *ctx, arg_empty *a, const char *mnemonic)
{
    output(ctx, mnemonic, "");
}
...
#define output_fcmp(C, PREFIX, SUFFIX)                                     \
    {                                                                      \
        (C)->info->fprintf_func((C)->info->stream,                         \
                                "%08x   %s%s\tfcc%d, f%d, f%d", (C)->insn, \
                                PREFIX, SUFFIX, a->cd, a->fj, a->fk);      \
    }
...
#define FCMP_INSN(suffix)                                                     \
    static bool trans_fcmp_cond_##suffix(DisasContext *ctx, arg_cff_fcond *a) \
    {                                                                         \
...
#define PCADD_INSN(name)                                          \
    ...
    {                                                             \
        output(ctx, #name, "r%d, %d # 0x%" PRIx64, a->rd, a->imm, \
               gen_##name(ctx->pc, a->imm));                      \
        return true;                                              \
    }
...
static void output_cv(DisasContext *ctx, arg_cv *a, const char *mnemonic)
{
...
#define output_vfcmp(C, PREFIX, SUFFIX)                                 \
    {                                                                   \
        (C)->info->fprintf_func((C)->info->stream,                      \
                                "%08x   %s%s\t%d, f%d, f%d", (C)->insn, \
                                PREFIX, SUFFIX, a->vd, a->vj, a->vk);   \
    }
...
#define LSX_FCMP_INSN(suffix)                                                  \
    static bool trans_vfcmp_cond_##suffix(DisasContext *ctx, arg_vvv_fcond *a) \
    {                                                                          \
