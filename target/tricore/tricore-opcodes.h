...
#define MASK_BITS_SHIFT(op, start, end) \
    (extract32(op, (start), (end) - (start) + 1))
#define MASK_BITS_SHIFT_SEXT(op, start, end) \
    (sextract32(op, (start), (end) - (start) + 1))
...
/* ABS Format */
#define MASK_OP_ABS_OFF18(op)                                           \
    (MASK_BITS_SHIFT(op, 16, 21) + (MASK_BITS_SHIFT(op, 28, 31) << 6) + \
...
/* B Format   */
#define MASK_OP_B_DISP24(op) \
    (MASK_BITS_SHIFT(op, 16, 31) + (MASK_BITS_SHIFT(op, 8, 15) << 16))
#define MASK_OP_B_DISP24_SEXT(op) \
    (MASK_BITS_SHIFT(op, 16, 31) + (MASK_BITS_SHIFT_SEXT(op, 8, 15) << 16))
/* BIT Format */
...
/* BO Format */
#define MASK_OP_BO_OFF10(op) \
    (MASK_BITS_SHIFT(op, 16, 21) + (MASK_BITS_SHIFT(op, 28, 31) << 6))
#define MASK_OP_BO_OFF10_SEXT(op) \
    (MASK_BITS_SHIFT(op, 16, 21) + (MASK_BITS_SHIFT_SEXT(op, 28, 31) << 6))
...
/* BOL Format */
#define MASK_OP_BOL_OFF16(op)                                             \
    ((MASK_BITS_SHIFT(op, 16, 21) + (MASK_BITS_SHIFT(op, 28, 31) << 6)) + \
     (MASK_BITS_SHIFT(op, 22, 27) << 10))
#define MASK_OP_BOL_OFF16_SEXT(op)                                        \
    ((MASK_BITS_SHIFT(op, 16, 21) + (MASK_BITS_SHIFT(op, 28, 31) << 6)) + \
...
#define MASK_OP_BRN_N(op) \
    (MASK_BITS_SHIFT(op, 12, 15) + (MASK_BITS_SHIFT(op, 7, 7) << 4))
