...
static int hax_open_device(hax_fd *fd)
{
    ...
    hDevice = CreateFile("\\\\.\\HAX", GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
int hax_populate_ram(uint64_t va, uint64_t size)
{
    ...
    if (hax_global.supports_64bit_ramblock) {
        ...
        ret = DeviceIoControl(hDeviceVM, HAX_VM_IOCTL_ADD_RAMBLOCK, &ramblock,
                              sizeof(ramblock), NULL, 0, &dSize,
                              (LPOVERLAPPED)NULL);
    } else {
        ...
        ret =
            DeviceIoControl(hDeviceVM, HAX_VM_IOCTL_ALLOC_RAM, &info,
                            sizeof(info), NULL, 0, &dSize, (LPOVERLAPPED)NULL);
    }
...
int hax_set_ram(uint64_t start_pa, uint32_t size, uint64_t host_va, int flags)
{
    ...
    ret = DeviceIoControl(hDeviceVM, HAX_VM_IOCTL_SET_RAM, &info, sizeof(info),
...
int hax_mod_version(struct hax_state *hax, struct hax_module_version *version)
{
    ...
    ret = DeviceIoControl(hDevice, HAX_IOCTL_VERSION, NULL, 0, version,
...
int hax_host_create_vm(struct hax_state *hax, int *vmid)
{
    ...
    ret = DeviceIoControl(hax->fd, HAX_IOCTL_CREATE_VM, NULL, 0, &vm_id,
                          sizeof(vm_id), &dSize, (LPOVERLAPPED)NULL);
    if (!ret) {
...
hax_fd hax_host_open_vm(struct hax_state *hax, int vm_id)
{
    ...
    hDeviceVM = CreateFile(vm_name, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                           CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
int hax_notify_qemu_version(hax_fd vm_fd, struct hax_qemu_version *qversion)
{
    ...
    ret = DeviceIoControl(vm_fd, HAX_VM_IOCTL_NOTIFY_QEMU_VERSION, qversion,
                          sizeof(struct hax_qemu_version), NULL, 0, &dSize,
                          (LPOVERLAPPED)NULL);
    if (!ret) {
...
int hax_host_create_vcpu(hax_fd vm_fd, int vcpuid)
{
    ...
    ret = DeviceIoControl(vm_fd, HAX_VM_IOCTL_VCPU_CREATE, &vcpuid,
                          sizeof(vcpuid), NULL, 0, &dSize, (LPOVERLAPPED)NULL);
    if (!ret) {
...
hax_fd hax_host_open_vcpu(int vmid, int vcpuid)
{
    ...
    hDeviceVCPU = CreateFile(devfs_path, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                             CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
int hax_host_setup_vcpu_channel(AccelCPUState *vcpu)
{
    ...
    ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_SETUP_TUNNEL, NULL, 0,
                          &info, sizeof(info), &dSize, (LPOVERLAPPED)NULL);
    if (!ret) {
...
int hax_vcpu_run(AccelCPUState *vcpu)
{
    ...
    ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_RUN, NULL, 0, NULL, 0,
...
int hax_sync_fpu(CPUArchState *env, struct fx_layout *fl, int set)
{
    ...
    if (set) {
        ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_SET_FPU, fl,
                              sizeof(*fl), NULL, 0, &dSize, (LPOVERLAPPED)NULL);
    } else {
        ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_GET_FPU, NULL, 0, fl,
                              sizeof(*fl), &dSize, (LPOVERLAPPED)NULL);
    }
    if (!ret) {
        return -EFAULT;
    } else {
...
int hax_sync_msr(CPUArchState *env, struct hax_msr_data *msrs, int set)
{
    ...
    if (set) {
        ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_SET_MSRS, msrs,
                              sizeof(*msrs), msrs, sizeof(*msrs), &dSize,
                              (LPOVERLAPPED)NULL);
    } else {
        ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_GET_MSRS, msrs,
                              sizeof(*msrs), msrs, sizeof(*msrs), &dSize,
                              (LPOVERLAPPED)NULL);
    }
    if (!ret) {
...
int hax_sync_vcpu_state(CPUArchState *env, struct vcpu_state_t *state, int set)
{
    ...
    if (set) {
        ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_SET_REGS, state,
                              sizeof(*state), NULL, 0, &dSize,
                              (LPOVERLAPPED)NULL);
    } else {
        ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_GET_REGS, NULL, 0, state,
                              sizeof(*state), &dSize, (LPOVERLAPPED)NULL);
    }
    if (!ret) {
        return -EFAULT;
    } else {
...
int hax_inject_interrupt(CPUArchState *env, int vector)
{
    ...
    ret = DeviceIoControl(hDeviceVCPU, HAX_VCPU_IOCTL_INTERRUPT, &vector,
                          sizeof(vector), NULL, 0, &dSize, (LPOVERLAPPED)NULL);
    if (!ret) {
        return -EFAULT;
    } else {
        return 0;
