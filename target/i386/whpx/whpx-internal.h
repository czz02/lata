...
#define LIST_WINHVPLATFORM_FUNCTIONS(X)                                       \
    X(HRESULT, WHvGetCapability,                                              \
      (WHV_CAPABILITY_CODE CapabilityCode, VOID * CapabilityBuffer,           \
       UINT32 CapabilityBufferSizeInBytes, UINT32 * WrittenSizeInBytes))      \
    ...
    X(HRESULT, WHvCreateVirtualProcessor,                                     \
      (WHV_PARTITION_HANDLE Partition, UINT32 VpIndex, UINT32 Flags))         \
    X(HRESULT, WHvDeleteVirtualProcessor,                                     \
      (WHV_PARTITION_HANDLE Partition, UINT32 VpIndex))                       \
    X(HRESULT, WHvRunVirtualProcessor,                                        \
      (WHV_PARTITION_HANDLE Partition, UINT32 VpIndex, VOID * ExitContext,    \
       UINT32 ExitContextSizeInBytes))                                        \
    X(HRESULT, WHvCancelRunVirtualProcessor,                                  \
      (WHV_PARTITION_HANDLE Partition, UINT32 VpIndex, UINT32 Flags))         \
    ...
    X(HRESULT, WHvSetVirtualProcessorRegisters,                               \
      (WHV_PARTITION_HANDLE Partition, UINT32 VpIndex,                        \
       const WHV_REGISTER_NAME *RegisterNames, UINT32 RegisterCount,          \
...
#define LIST_WINHVPLATFORM_FUNCTIONS_SUPPLEMENTAL(X)                      \
    ...
    X(HRESULT, WHvRequestInterrupt,                                       \
      (WHV_PARTITION_HANDLE Partition, WHV_INTERRUPT_CONTROL * Interrupt, \
...
#define LIST_WINHVEMULATION_FUNCTIONS(X)                                   \
    ...
    X(HRESULT, WHvEmulatorTryMmioEmulation,                                \
      (WHV_EMULATOR_HANDLE Emulator, VOID * Context,                       \
       const WHV_VP_EXIT_CONTEXT *VpContext,                               \
