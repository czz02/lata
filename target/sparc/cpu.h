...
#define CPU_DEFAULT_FEATURES                                   \
    (CPU_FEATURE_FLOAT | CPU_FEATURE_SWAP | CPU_FEATURE_MUL |  \
     CPU_FEATURE_DIV | CPU_FEATURE_FLUSH | CPU_FEATURE_FSQRT | \
     CPU_FEATURE_FMUL | CPU_FEATURE_VIS1 | CPU_FEATURE_VIS2 |  \
...
int sparc_cpu_memory_rw_debug(CPUState *cpu, vaddr addr, uint8_t *buf, int len,
                              bool is_write);
#endif
...
static inline int cpu_mmu_index(CPUSPARCState *env, bool ifetch)
{
    ...
    if (ifetch ? (env->lsu & IMMU_E) == 0 || (env->pstate & PS_RED) != 0 :
                 (env->lsu & DMMU_E) == 0) {
        return MMU_PHYS_IDX;
...
{
    uint64_t tstate = (cpu_get_ccr(env) << 32) | ((env->asi & 0xff) << 24) |
                      ((env->pstate & 0xf3f) << 8) | cpu_get_cwp64(env);
