...
void helper_put(uint32_t id, uint32_t ctrl, uint32_t data)
{
    ...
    qemu_log_mask(LOG_UNIMP,
                  ...
                  id, data, test ? "t" : "", nonblock ? "n" : "",
                  exception ? "e" : "", control ? "c" : "", atomic ? "a" : "");
}
...
static void update_fpu_flags(CPUMBState *env, int flags, uintptr_t ra)
{
    ...
    if (raise && (env_archcpu(env)->cfg.pvr_regs[2] & PVR2_FPU_EXC_MASK) &&
        (env->msr & MSR_EE)) {
...
void helper_stackprot(CPUMBState *env, target_ulong addr)
{
    if (addr < env->slr || addr > env->shr) {
        ...
        qemu_log_mask(CPU_LOG_INT,
                      "Stack protector violation at " TARGET_FMT_lx " %x %x\n",
                               ...
                               MemTxResult response, uintptr_t retaddr)
{
    ...
    qemu_log_mask(
        CPU_LOG_INT,
        "Transaction failed: vaddr 0x%" VADDR_PRIx " physaddr 0x" HWADDR_FMT_plx
