...
uint64_t HELPER(iwmmxt_maddsq)(uint64_t a, uint64_t b)
{
    a = ((EXTEND16S((a >> 0) & 0xffff) * EXTEND16S((b >> 0) & 0xffff) +
         ...
         0xffffffff) |
        ((uint64_t)(EXTEND16S((a >> 32) & 0xffff) *
                        EXTEND16S((b >> 32) & 0xffff) +
...
#define SADB(SHR) abs((int)((a >> SHR) & 0xff) - (int)((b >> SHR) & 0xff))
    return SADB(0) + SADB(8) + SADB(16) + SADB(24) + SADB(32) + SADB(40) +
           SADB(48) + SADB(56);
#undef SADB
}
...
{
#define MULS(SHR)                                    \
    ((uint64_t)(((EXTEND16S((a >> SHR) & 0xffff) *   \
                  EXTEND16S((b >> SHR) & 0xffff)) >> \
                 0) &                                \
                0xffff)                              \
     << SHR)
...
{
#define MULU(SHR)                                                        \
    ((uint64_t)(((((a >> SHR) & 0xffff) * ((b >> SHR) & 0xffff)) >> 0) & \
                0xffff)                                                  \
     << SHR)
...
{
#define MULU(SHR)                                                         \
    ((uint64_t)(((((a >> SHR) & 0xffff) * ((b >> SHR) & 0xffff)) >> 16) & \
                0xffff)                                                   \
     << SHR)
    return MULU(0) | MULU(16) | MULU(32) | MULU(48);
#undef MULU
...
{
#define MACU(SHR) \
    ((uint32_t)((a >> SHR) & 0xffff) * (uint32_t)((b >> SHR) & 0xffff))
...
#define IWMMXT_OP_UNPACK(S, SH0, SH1, SH2, SH3)                              \
    ...
    uint64_t HELPER(glue(iwmmxt_unpack, glue(S, w)))(CPUARMState * env,      \
                                                     uint64_t a, uint64_t b) \
    {                                                                        \
        a = (((a >> SH0) & 0xffff) << 0) | (((b >> SH0) & 0xffff) << 16) |   \
            (((a >> SH2) & 0xffff) << 32) | (((b >> SH2) & 0xffff) << 48);   \
        env->iwmmxt.cregs[ARM_IWMMXT_wCASF] =                                \
            NZBIT8(a >> 0, 0) | NZBIT8(a >> 16, 1) | NZBIT8(a >> 32, 2) |    \
            NZBIT8(a >> 48, 3);                                              \
        return a;                                                            \
    }                                                                        \
    ...
    uint64_t HELPER(glue(iwmmxt_unpack, glue(S, ub)))(CPUARMState * env,     \
                                                      uint64_t x)            \
    {                                                                        \
        x = (((x >> SH0) & 0xff) << 0) | (((x >> SH1) & 0xff) << 16) |       \
            (((x >> SH2) & 0xff) << 32) | (((x >> SH3) & 0xff) << 48);       \
        env->iwmmxt.cregs[ARM_IWMMXT_wCASF] =                                \
            NZBIT16(x >> 0, 0) | NZBIT16(x >> 16, 1) | NZBIT16(x >> 32, 2) | \
            NZBIT16(x >> 48, 3);                                             \
        return x;                                                            \
    }                                                                        \
    ...
    uint64_t HELPER(glue(iwmmxt_unpack, glue(S, sb)))(CPUARMState * env,     \
                                                      uint64_t x)            \
    {                                                                        \
        x = ((uint64_t)EXTEND8H((x >> SH0) & 0xff) << 0) |                   \
    ...
    uint64_t HELPER(glue(iwmmxt_unpack, glue(S, sw)))(CPUARMState * env,     \
                                                      uint64_t x)            \
    {                                                                        \
        x = ((uint64_t)EXTEND16((x >> SH0) & 0xffff) << 0) |                 \
...
#define IWMMXT_OP_CMP(SUFF, Tb, Tw, Tl, O)                                     \
    uint64_t HELPER(glue(iwmmxt_, glue(SUFF, b)))(CPUARMState * env,           \
                                                  uint64_t a, uint64_t b)      \
    {                                                                          \
        a = CMP(0, Tb, O, 0xff) | CMP(8, Tb, O, 0xff) | CMP(16, Tb, O, 0xff) | \
            CMP(24, Tb, O, 0xff) | CMP(32, Tb, O, 0xff) |                      \
            CMP(40, Tb, O, 0xff) | CMP(48, Tb, O, 0xff) |                      \
            CMP(56, Tb, O, 0xff);                                              \
        env->iwmmxt.cregs[ARM_IWMMXT_wCASF] =                                  \
            NZBIT8(a >> 0, 0) | NZBIT8(a >> 8, 1) | NZBIT8(a >> 16, 2) |       \
            NZBIT8(a >> 24, 3) | NZBIT8(a >> 32, 4) | NZBIT8(a >> 40, 5) |     \
    ...
    {                                                                          \
        ...
        env->iwmmxt.cregs[ARM_IWMMXT_wCASF] =                                  \
            NZBIT16(a >> 0, 0) | NZBIT16(a >> 16, 1) | NZBIT16(a >> 32, 2) |   \
            NZBIT16(a >> 48, 3);                                               \
        return a;                                                              \
    }                                                                          \
...
#define CMP(SHR, TYPE, OPER, MASK)                                \
    ((((TYPE)((a >> SHR) & MASK) OPER(TYPE)((b >> SHR) & MASK)) ? \
          (uint64_t)MASK :                                        \
          0)                                                      \
     << SHR)
...
#undef CMP
#define CMP(SHR, TYPE, OPER, MASK)                                         \
    ((((TYPE)((a >> SHR) & MASK) OPER(TYPE)((b >> SHR) & MASK)) ? a : b) & \
     ((uint64_t)MASK << SHR))
...
#undef CMP
#define CMP(SHR, TYPE, OPER, MASK)                                          \
    ((uint64_t)(((TYPE)((a >> SHR) & MASK) OPER(TYPE)((b >> SHR) & MASK)) & \
                MASK)                                                       \
     << SHR)
...
#define IWMMXT_OP_AVGB(r)                                                      \
    ...
    {                                                                          \
        const int round = r;                                                   \
        a = AVGB(0) | AVGB(8) | AVGB(16) | AVGB(24) | AVGB(32) | AVGB(40) |    \
            AVGB(48) | AVGB(56);                                               \
        env->iwmmxt.cregs[ARM_IWMMXT_wCASF] =                                  \
...
{
    arg &= 0xff;
    return ((uint64_t)arg << 0) | ((uint64_t)arg << 8) | ((uint64_t)arg << 16) |
           ((uint64_t)arg << 24) | ((uint64_t)arg << 32) |
           ((uint64_t)arg << 40) | ((uint64_t)arg << 48) |
           ((uint64_t)arg << 56);
}
...
{
    arg &= 0xffff;
    return ((uint64_t)arg << 0) | ((uint64_t)arg << 16) |
...
{
    return ((x >> 0) & 0xff) + ((x >> 8) & 0xff) + ((x >> 16) & 0xff) +
           ((x >> 24) & 0xff) + ((x >> 32) & 0xff) + ((x >> 40) & 0xff) +
           ((x >> 48) & 0xff) + ((x >> 56) & 0xff);
}

uint64_t HELPER(iwmmxt_addcw)(uint64_t x)
{
    return ((x >> 0) & 0xffff) + ((x >> 16) & 0xffff) + ((x >> 32) & 0xffff) +
           ((x >> 48) & 0xffff);
}
...
{
    return ((x >> 7) & 0x01) | ((x >> 14) & 0x02) | ((x >> 21) & 0x04) |
           ((x >> 28) & 0x08) | ((x >> 35) & 0x10) | ((x >> 42) & 0x20) |
           ((x >> 49) & 0x40) | ((x >> 56) & 0x80);
}

uint32_t HELPER(iwmmxt_msbw)(uint64_t x)
{
    return ((x >> 15) & 0x01) | ((x >> 30) & 0x02) | ((x >> 45) & 0x04) |
           ((x >> 52) & 0x08);
}
...
{
    x = ((((x & (0xffffll << 0)) >> n) | ((x & (0xffffll << 0)) << (16 - n))) &
         (0xffffll << 0)) |
        ((((x & (0xffffll << 16)) >> n) |
          ((x & (0xffffll << 16)) << (16 - n))) &
         (0xffffll << 16)) |
...
uint64_t HELPER(iwmmxt_muladdsw)(uint64_t c, uint32_t a, uint32_t b)
{
    ...
    c +=
        EXTEND32(EXTEND16S((a >> 16) & 0xffff) * EXTEND16S((b >> 16) & 0xffff));
    return c;
}
