...
#define get_field(reg, mask) \
    (((reg) & (uint64_t)(mask)) / ((mask) & ~((mask) << 1)))
#define set_field(reg, mask, val)  \
    (((reg) & ~(uint64_t)(mask)) | \
     (((uint64_t)(val) * ((mask) & ~((mask) << 1))) & (uint64_t)(mask)))
...
#define MMTE_MASK                                            \
    (MMTE_U_PM_ENABLE | MMTE_U_PM_CURRENT | MMTE_U_PM_INSN | \
     MMTE_S_PM_ENABLE | MMTE_S_PM_CURRENT | MMTE_S_PM_INSN | \
     MMTE_M_PM_ENABLE | MMTE_M_PM_CURRENT | MMTE_M_PM_INSN | MMTE_PM_XS_BITS)
...
#define SMTE_MASK                                            \
    (SMTE_U_PM_ENABLE | SMTE_U_PM_CURRENT | SMTE_U_PM_INSN | \
     SMTE_S_PM_ENABLE | SMTE_S_PM_CURRENT | SMTE_S_PM_INSN | SMTE_PM_XS_BITS)
