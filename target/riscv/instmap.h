...
#define MASK_OP_ATOMIC(op) \
    (MASK_OP_MAJOR(op) | (op & ((0x7 << 12) | (0x7F << 25))))
#define MASK_OP_ATOMIC_NO_AQ_RL_SZ(op) (MASK_OP_MAJOR(op) | (op & (0x1F << 27)))
...
#define GET_B_IMM(inst)                                             \
    ((extract32(inst, 8, 4) << 1) | (extract32(inst, 25, 6) << 5) | \
     (extract32(inst, 7, 1) << 11) | (sextract64(inst, 31, 1) << 12))

#define GET_STORE_IMM(inst) \
    ((extract32(inst, 7, 5)) | (sextract64(inst, 25, 7) << 5))

#define GET_JAL_IMM(inst)                                              \
    ((extract32(inst, 21, 10) << 1) | (extract32(inst, 20, 1) << 11) | \
     (extract32(inst, 12, 8) << 12) | (sextract64(inst, 31, 1) << 20))
...
#define GET_C_ADDI4SPN_IMM(inst)                                   \
    ((extract32(inst, 6, 1) << 2) | (extract32(inst, 5, 1) << 3) | \
     (extract32(inst, 11, 2) << 4) | (extract32(inst, 7, 4) << 6))
#define GET_C_ADDI16SP_IMM(inst)                                   \
    ((extract32(inst, 6, 1) << 4) | (extract32(inst, 2, 1) << 5) | \
     (extract32(inst, 5, 1) << 6) | (extract32(inst, 3, 2) << 7) | \
     (sextract64(inst, 12, 1) << 9))
#define GET_C_LWSP_IMM(inst)                                        \
    ((extract32(inst, 4, 3) << 2) | (extract32(inst, 12, 1) << 5) | \
     (extract32(inst, 2, 2) << 6))
#define GET_C_LDSP_IMM(inst)                                        \
    ((extract32(inst, 5, 2) << 3) | (extract32(inst, 12, 1) << 5) | \
     (extract32(inst, 2, 3) << 6))
#define GET_C_SWSP_IMM(inst) \
    ((extract32(inst, 9, 4) << 2) | (extract32(inst, 7, 2) << 6))
#define GET_C_SDSP_IMM(inst) \
    ((extract32(inst, 10, 3) << 3) | (extract32(inst, 7, 3) << 6))
#define GET_C_LW_IMM(inst)                                          \
    ((extract32(inst, 6, 1) << 2) | (extract32(inst, 10, 3) << 3) | \
     (extract32(inst, 5, 1) << 6))
#define GET_C_LD_IMM(inst) \
    ((extract16(inst, 10, 3) << 3) | (extract16(inst, 5, 2) << 6))
#define GET_C_SW_IMM(inst) GET_C_LW_IMM(inst)
#define GET_C_SD_IMM(inst) GET_C_LD_IMM(inst)
#define GET_C_J_IMM(inst)                                           \
    ((extract32(inst, 3, 3) << 1) | (extract32(inst, 11, 1) << 4) | \
     (extract32(inst, 2, 1) << 5) | (extract32(inst, 7, 1) << 6) |  \
     (extract32(inst, 6, 1) << 7) | (extract32(inst, 9, 2) << 8) |  \
     (extract32(inst, 8, 1) << 10) | (sextract64(inst, 12, 1) << 11))
#define GET_C_B_IMM(inst)                                           \
    ((extract32(inst, 3, 2) << 1) | (extract32(inst, 10, 2) << 3) | \
     (extract32(inst, 2, 1) << 5) | (extract32(inst, 5, 2) << 6) |  \
