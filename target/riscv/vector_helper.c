                            ...
                            target_ulong s2)
{
    ...
    target_ulong reserved =
        s2 & MAKE_64BIT_MASK(R_VTYPE_RESERVED_SHIFT,
                             xlen - 1 - R_VTYPE_RESERVED_SHIFT);

    if (lmul & 4) {
        /* Fractional LMUL - check LMUL * VLEN >= SEW */
        if (lmul == 4 || cpu->cfg.vlen >> (8 - lmul) < sew) {
            vill = true;
        }
    }
                                            ...
                                            uint32_t esz)
{
    ...
    int8_t emul = ctzl(esz) - ctzl(sew) + vext_lmul(desc) < 0 ?
                      0 :
...
#define GEN_VEXT_LD_ELEM(NAME, ETYPE, H, LDSUF)                                \
    static void NAME(CPURISCVState *env, abi_ptr addr, uint32_t idx, void *vd, \
                     uintptr_t retaddr)                                        \
    {                                                                          \
...
static void vext_set_tail_elems_1s(target_ulong vl, void *vd, uint32_t desc,
                                   uint32_t nf, uint32_t esz,
                                   uint32_t max_elems)
{
...
#define GEN_VEXT_LD_US(NAME, ETYPE, LOAD_FN)                              \
    ...
    void HELPER(NAME)(void *vd, void *v0, target_ulong base,              \
                      CPURISCVState *env, uint32_t desc)                  \
    {                                                                     \
        vext_ldst_us(vd, base, env, desc, LOAD_FN, ctzl(sizeof(ETYPE)),   \
                     env->vl, GETPC());                                   \
    }
...
#define GEN_VEXT_ST_US(NAME, ETYPE, STORE_FN)                              \
    ...
    void HELPER(NAME)(void *vd, void *v0, target_ulong base,               \
                      CPURISCVState *env, uint32_t desc)                   \
    {                                                                      \
        vext_ldst_us(vd, base, env, desc, STORE_FN, ctzl(sizeof(ETYPE)),   \
                     env->vl, GETPC());                                    \
    }
 ...
 */
void HELPER(vlm_v)(void *vd, void *v0, target_ulong base, CPURISCVState *env,
                   uint32_t desc)
{
    /* evl = ceil(vl/8) */
...
void HELPER(vsm_v)(void *vd, void *v0, target_ulong base, CPURISCVState *env,
                   uint32_t desc)
{
    /* evl = ceil(vl/8) */
    uint8_t evl = (env->vl + 7) >> 3;
...
#define GEN_VEXT_LD_INDEX(NAME, ETYPE, INDEX_FN, LOAD_FN)                \
    void HELPER(NAME)(void *vd, void *v0, target_ulong base, void *vs2,  \
                      CPURISCVState *env, uint32_t desc)                 \
    {                                                                    \
        vext_ldst_index(vd, v0, base, vs2, env, desc, INDEX_FN, LOAD_FN, \
                        ctzl(sizeof(ETYPE)), GETPC());                   \
    }
...
#define GEN_VEXT_ST_INDEX(NAME, ETYPE, INDEX_FN, STORE_FN)                \
    void HELPER(NAME)(void *vd, void *v0, target_ulong base, void *vs2,   \
                      CPURISCVState *env, uint32_t desc)                  \
    {                                                                     \
        vext_ldst_index(vd, v0, base, vs2, env, desc, INDEX_FN, STORE_FN, \
                        ctzl(sizeof(ETYPE)), GETPC());                    \
    }
...
#define GEN_VEXT_LDFF(NAME, ETYPE, LOAD_FN)                              \
    void HELPER(NAME)(void *vd, void *v0, target_ulong base,             \
                      CPURISCVState *env, uint32_t desc)                 \
    {                                                                    \
        vext_ldff(vd, v0, base, env, desc, LOAD_FN, ctzl(sizeof(ETYPE)), \
                  GETPC());                                              \
    }
 ...
 */
static void vext_ldst_whole(void *vd, target_ulong base, CPURISCVState *env,
                            uint32_t desc, vext_ldst_elem_fn *ldst_elem,
                            uint32_t log2_esz, uintptr_t ra)
{
...
#define GEN_VEXT_LD_WHOLE(NAME, ETYPE, LOAD_FN)                            \
    void HELPER(NAME)(void *vd, target_ulong base, CPURISCVState *env,     \
                      uint32_t desc)                                       \
    {                                                                      \
        vext_ldst_whole(vd, base, env, desc, LOAD_FN, ctzl(sizeof(ETYPE)), \
                        GETPC());                                          \
    }
...
#define GEN_VEXT_ST_WHOLE(NAME, ETYPE, STORE_FN)                            \
    void HELPER(NAME)(void *vd, target_ulong base, CPURISCVState *env,      \
                      uint32_t desc)                                        \
    {                                                                       \
        vext_ldst_whole(vd, base, env, desc, STORE_FN, ctzl(sizeof(ETYPE)), \
                        GETPC());                                           \
    }
...
static void do_vext_vv(void *vd, void *v0, void *vs1, void *vs2,
                       CPURISCVState *env, uint32_t desc, opivv2_fn *fn,
                       uint32_t esz)
{
    uint32_t vm = vext_vm(desc);
    uint32_t vl = env->vl;
...
#define DO_MADC(N, M, C) \
    (C ? (__typeof(N))(N + M + 1) <= N : (__typeof(N))(N + M) < N)
...
#define GEN_VEXT_CMP_VX(NAME, ETYPE, H, DO_OP)                            \
    ...
    {                                                                     \
        ...
        for (i = env->vstart; i < vl; i++) {                              \
            ...
            vext_set_elem_mask(vd, i, DO_OP(s2, (ETYPE)(target_long)s1)); \
        }                                                                 \
        env->vstart = 0;                                                  \
        /*                                                                \
         * mask destination register are always tail-agnostic             \
...
#define GEN_VEXT_VMV_VV(NAME, ETYPE, H)                                       \
    void HELPER(NAME)(void *vd, void *vs1, CPURISCVState *env, uint32_t desc) \
    {                                                                         \
        uint32_t vl = env->vl;                                                \
        uint32_t esz = sizeof(ETYPE);                                         \
        uint32_t total_elems = vext_get_total_elems(env, desc, esz);          \
...
#define GEN_VEXT_VMERGE_VX(NAME, ETYPE, H)                                    \
    ...
    {                                                                         \
        ...
        for (i = env->vstart; i < vl; i++) {                                  \
            ETYPE s2 = *((ETYPE *)vs2 + H(i));                                \
            ETYPE d = (!vext_elem_mask(v0, i) ? s2 : (ETYPE)(target_long)s1); \
            *((ETYPE *)vd + H(i)) = d;                                        \
        }                                                                     \
        env->vstart = 0;                                                      \
        /* set tail elements to 1s */                                         \
...
#define OPIVV2_RM(NAME, TD, T1, T2, TX1, TX2, HD, HS1, HS2, OP)         \
    static inline void do_##NAME(void *vd, void *vs1, void *vs2, int i, \
                                 CPURISCVState *env, int vxrm)          \
    {                                                                   \
        TX1 s1 = *((T1 *)vs1 + HS1(i));                                 \
        TX2 s2 = *((T2 *)vs2 + HS2(i));                                 \
...
static inline void vext_vv_rm_1(void *vd, void *v0, void *vs1, void *vs2,
...
static inline void vext_vv_rm_2(void *vd, void *v0, void *vs1, void *vs2,
                                CPURISCVState *env, uint32_t desc,
...
#define OPIVX2_RM(NAME, TD, T1, T2, TX1, TX2, HD, HS2, OP)                   \
    static inline void do_##NAME(void *vd, target_long s1, void *vs2, int i, \
                                 CPURISCVState *env, int vxrm)               \
    {                                                                        \
        TX2 s2 = *((T2 *)vs2 + HS2(i));                                      \
...
static inline void vext_vx_rm_1(void *vd, void *v0, target_long s1, void *vs2,
                                CPURISCVState *env, uint32_t vl, uint32_t vm,
...
static inline void vext_vx_rm_2(void *vd, void *v0, target_long s1, void *vs2,
                                CPURISCVState *env, uint32_t desc,
...
static inline uint8_t vssrl8(CPURISCVState *env, int vxrm, uint8_t a, uint8_t b)
{
...
static inline int8_t vssra8(CPURISCVState *env, int vxrm, int8_t a, int8_t b)
{
    uint8_t round, shift = b & 0x7;
...
static inline int8_t vnclip8(CPURISCVState *env, int vxrm, int16_t a, int8_t b)
{
    uint8_t round, shift = b & 0xf;
    int16_t res;
...
{
    return float64_add(float32_to_float64(a, s), float32_to_float64(b, s), s);
}
...
{
    return float64_sub(float32_to_float64(a, s), float32_to_float64(b, s), s);
}
...
{
    return float64_mul(float32_to_float64(a, s), float32_to_float64(b, s), s);
}
...
{
    return float16_muladd(
        a, b, d, float_muladd_negate_c | float_muladd_negate_product, s);
}
...
{
    return float16_muladd(
        d, b, a, float_muladd_negate_c | float_muladd_negate_product, s);
}
...
{
    return float64_muladd(float32_to_float64(a, s), float32_to_float64(b, s), d,
                          0, s);
}
...
{
    return float32_muladd(bfloat16_to_float32(a, s), bfloat16_to_float32(b, s),
                          d, 0, s);
}
...
{
    return float32_muladd(
        float16_to_float32(a, true, s), float16_to_float32(b, true, s), d,
...
#define OPFVV1(NAME, TD, T2, TX2, HD, HS2, OP)                            \
    static void do_##NAME(void *vd, void *vs2, int i, CPURISCVState *env) \
    {                                                                     \
        TX2 s2 = *((T2 *)vs2 + HS2(i));                                   \
...
static uint64_t frsqrt7(uint64_t f, int exp_size, int frac_size)
{
    ...
    int idx =
        ((exp & 1) << (precision - 1)) | (frac >> (frac_size - precision + 1));
...
static float16 frsqrt7_h(float16 f, float_status *s)
{
     ...
     */
    if (float16_is_signaling_nan(f, s) || (float16_is_infinity(f) && sign) ||
...
static float32 frsqrt7_s(float32 f, float_status *s)
{
     ...
     * frsqrt7(-subnormal) = canonical NaN
     */
    if (float32_is_signaling_nan(f, s) || (float32_is_infinity(f) && sign) ||
...
static float64 frsqrt7_d(float64 f, float_status *s)
{
     ...
     * frsqrt7(-subnormal) = canonical NaN
     */
    if (float64_is_signaling_nan(f, s) || (float64_is_infinity(f) && sign) ||
 ...
 * https://github.com/riscv/riscv-v-spec/blob/master/recip.c
 */
static uint64_t frec7(uint64_t f, int exp_size, int frac_size, float_status *s)
{
    uint64_t sign = extract64(f, frac_size + exp_size, 1);
    uint64_t exp = extract64(f, frac_size, exp_size);
    uint64_t frac = extract64(f, 0, frac_size);
    ...
    if (exp == 0 && frac != 0) { /* subnormal */
        ...
        if (exp != 0 && exp != UINT64_MAX) {
            ...
            s->float_exception_flags |=
                (float_flag_inexact | float_flag_overflow);
...
#define GEN_VEXT_CMP_VF(NAME, ETYPE, H, DO_OP)                                \
    ...
    {                                                                         \
        ...
        for (i = env->vstart; i < vl; i++) {                                  \
            ...
            vext_set_elem_mask(vd, i, DO_OP(s2, (ETYPE)s1, &env->fp_status)); \
        }                                                                     \
        env->vstart = 0;                                                      \
        /*                                                                    \
         * mask destination register are always tail-agnostic                 \
...
{
    FloatRelation compare = float16_compare(a, b, s);
    return compare == float_relation_greater || compare == float_relation_equal;
}
...
#define GEN_VFMERGE_VF(NAME, ETYPE, H)                                         \
    ...
    {                                                                          \
        ...
        for (i = env->vstart; i < vl; i++) {                                   \
            ETYPE s2 = *((ETYPE *)vs2 + H(i));                                 \
            *((ETYPE *)vd + H(i)) = (!vm && !vext_elem_mask(v0, i) ? s2 : s1); \
        }                                                                      \
        env->vstart = 0;                                                       \
        /* set tail elements to 1s */                                          \
        vext_set_elems_1s(vd, vta, vl *esz, total_elems *esz);                 \
...
void HELPER(vmvr_v)(void *vd, void *vs2, CPURISCVState *env, uint32_t desc)
{
    ...
    memcpy((uint8_t *)vd + H1(i), (uint8_t *)vs2 + H1(i), maxsz - startb);
